...
//---------------------------------measure gap beween paddle & ball---------------------------------//
  ...
  if (circleDistanceX > (paddle.getPaddleWidth()/2 + ball.getDiameter()/2)){    // x gap too large
    return false; 
  }
  if (circleDistanceY > (paddle.getPaddleHeight()/2 + ball.getDiameter()/2)){   // y gap too large
    return false;
  }
  if (circleDistanceX <= (paddle.getPaddleWidth()/2)){                          // x gap small enough
    return true;
  }
  if (circleDistanceY <= (paddle.getPaddleHeight()/2)){                         // y gap small enough
    return true;
  }
  
//---------------------------Pythagorean Theorem tests corner collision----------------------------//  
  float cornerDistance = pow(circleDistanceX - paddle.getPaddleWidth()/2, 2) +  // x^2
                         pow(circleDistanceY - paddle.getPaddleHeight()/2, 2);  // y^2
  
  if (cornerDistance <= pow(ball.getDiameter()/2, 2)){  // if cornerDistance < the radius of the ball
    return true;
  }
  else{
    return false;
